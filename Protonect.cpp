...
const int METHODMODE = 1;       // 0 -> 従来手法,1 -> 新手法
...
Mat detectFaceInImage(Mat &image,string &cascade_file)
{
	...
	if(faces.size() != 0)
    {
        ...
        else if(METHODMODE == 1)
        {
            ...
            if(!dets.empty())
            {
                ...
                //右目
                    ...
                    imshow("righteye",trimming_righteye);
                ...
                res = pyWrapper_proposed(count);//python実行
            }
        }
        ...
        if(res == 1)
        {
            ...
            // putText(image,"face",Point(faces[0].x,faces[0].y),FONT_HERSHEY_SIMPLEX,0.4,Scalar(0,0,200),1,CV_AA);
        }
        else
        {
            cout << "not face"<< endl;
